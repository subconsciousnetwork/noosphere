...
use noosphere_storage::{BlockStore, BlockStoreRetry, MemoryStore, UcanStore};
...
async fn start_name_system_server<S: BlockStore + Clone + 'static>(
    store: S,
    listener: TcpListener,
) -> Result<JoinHandle<()>> {
    Ok(tokio::spawn(async move {
        let db = UcanStore(store.clone());
        let mut network = NameSystemNetwork::generate_without_validation(2, db)
            .await
            .unwrap();

...
impl SphereWriter {
        ...
        R: AsyncFileBody,
    {
        ...
        if self.is_root_writer() {
            ...
            self.symlink_content(&file.memo_version, &file_path).await?;
        }
    ...
    pub async fn symlink_content(&self, memo_cid: &Cid, file_path: &PathBuf) -> Result<()> {
        ...
        let relative_peer_content_path = diff_paths(&self.paths.peer_content(memo_cid), file_path)
            .ok_or_else(|| {
                anyhow!(
    ...
    pub async fn symlink_peer(&self, peer: &Did, version: &Cid, petname: &str) -> Result<()> {
        ...
        let relative_peer_source =
            diff_paths(self.paths.peer(peer, version), &absolute_peer_destination)

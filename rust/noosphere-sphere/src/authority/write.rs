    ...
    S: Storage + 'static,
{
    ...
    // TODO: We should probably enforce that each [Did] only gets one authorization,
    // from a hygeine perspective; elsewhere we need to assume multiple authorizations
    // for the same [Did] are possible.
    async fn authorize(&mut self, name: &str, identity: &Did) -> Result<Authorization> {

...
) -> Result<StreamBody<Pin<Box<dyn Stream<Item = Result<Bytes, std::io::Error>> + Send>>>, StatusCode>
where
    ...
    if let Some(since) = since {
        ...
        if is_allowed_to_replicate_incrementally(&since_memo, &latest_memo) {
            ...
            return Ok(StreamBody::new(Box::pin(car_stream(
                vec![memo_version],
                memo_history_stream(store, &memo_version.into(), Some(&since.into())),
            ))));
        } else {
            error!("Suggested older version {since} is not an ancestor of {memo_version}");
...
mod tests {
    ...
    async fn it_detects_forked_lineages_of_a_sphere_by_revoked_authors() -> Result<()> {
        ...
        let rogue_author = Author {
            ...
            authorization: Some(Authorization::Cid(delegation.jwt.clone())),
        };

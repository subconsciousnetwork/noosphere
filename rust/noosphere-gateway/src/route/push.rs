    ...
    S: Storage + 'static,
{
    pub async fn invoke(mut self) -> Result<PushResponse, PushError> {
        ...
        // let sphere_context = self.sphere_context.sphere_context_mut().await?;

        // self.verify_history(&mut sphere_context).await?;
        // self.incorporate_history(&mut sphere_context).await?;
        // self.synchronize_names(&mut sphere_context).await?;

...
mod inner {
    ...
    use noosphere_storage::{PerformanceStats, PerformanceStorage, SphereDb, Storage};
    use rand::Rng;
    ...
    pub type ActiveStoragePrimitive = noosphere_storage::SledStorage;
    #[cfg(all(not(target_arch = "wasm32"), feature = "rocksdb"))]
    pub type ActiveStoragePrimitive = noosphere_storage::RocksDbStorage;
    #[cfg(all(
        not(target_arch = "wasm32"),
        feature = "sqlite",
        not(feature = "rocksdb")
    ))]
    pub type ActiveStoragePrimitive = noosphere_storage::SqliteStorage;
    #[cfg(target_arch = "wasm32")]
    pub type ActiveStoragePrimitive = noosphere_storage::IndexedDbStorage;

    pub type ActiveStorageType = PerformanceStorage<ActiveStoragePrimitive>;
    ...
    pub async fn log_perf_stats(stats: &PerformanceStats) {
    ...
    pub async fn bench_sphere_writing_long_history() {
    ...
    pub async fn bench_sphere_writing_large_files() {
        let mut storage = BenchmarkStorage::new().await.unwrap();
        output!("Testing {}", storage.name);
...
#[cfg(all(target_arch = "wasm32", feature = "performance"))]
...
#[cfg(all(not(target_arch = "wasm32"), feature = "performance"))]
...
fn main() {
    initialize_tracing(None);
    panic!("Enable `performance` feature to run benchmarks.");
}

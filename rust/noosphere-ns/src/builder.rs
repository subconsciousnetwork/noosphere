use crate::{client::NameSystemClient, dht::DhtConfig, NameSystem, NameSystemKeyMaterial};
    ...
    S: UcanJwtStore + 'static,
{
    ...
    /// A [UcanStoreJwt] that can be used to fetch proofs when
    /// validating records.
    ...
    pub async fn build(mut self) -> Result<NameSystem> {
        ...
        let ns = NameSystem::new(&key_material, self.dht_config.clone(), store)?;

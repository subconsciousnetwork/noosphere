...
/// use noosphere_ns::{BOOTSTRAP_PEERS, NameSystem, NameSystemBuilder};
    ...
    K: DHTKeyMaterial + 'static,
{
    ...
    pub async fn build(mut self) -> Result<NameSystem> {
        ...
        let ns = NameSystem::new(
            &key_material,
            store,
            self.dht_config.clone(),
        )?;

        if let Some(listening_address) = self.listening_address {
            ns.start_listening(listening_address).await?;
        }
...
mod tests {
    ...
    async fn test_name_system_builder() -> Result<(), anyhow::Error> {
        ...
        assert_eq!(
            ns.addresses().await?.first().unwrap(),
